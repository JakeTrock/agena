...
const (
	...
	rateLimitPhrase  = "Rate limit reached"
)
...
// rateLimitError indicates Gemini returned a rate limit message
...
type RunnerOptions struct {
	...
	GeminiCommand string        // Gemini command (overrides task.yaml)
}
...
type Runner struct {
	...
	geminiLogger  *GeminiLogger
	geminiStats   *SessionStats
	stopRequested bool
	backoffLevel  int
...
func NewRunner(env *Environment, taskName string, opts RunnerOptions) (*Runner, error) {
	...
	var geminiLogger *GeminiLogger
	if !opts.DryRun {
		geminiLogger, err = NewGeminiLogger(task.Dir)
		if err != nil {
			return nil, fmt.Errorf("failed to create gemini logger: %w", err)
		}
	}
...
func (r *Runner) Run() error {
	// Verify gemini command exists (skip in dry-run)
	// Use the same precedence as execution: CLI override > task-level > global
	if !r.opts.DryRun {
		geminiCmd := r.opts.GeminiCommand
		if geminiCmd == "" {
			geminiCmd = r.task.GeminiCommand
		}
		if geminiCmd == "" {
			geminiCmd = r.env.Config.GeminiCommand
		}
		if err := CheckGeminiCommand(geminiCmd); err != nil {
			return err
		}
	}
	...
	logPath := filepath.Join(r.task.Dir, "gemini.log")
	...
	if r.geminiLogger != nil {
		r.geminiLogger.Close()
	}
...
func (r *Runner) runIteration() (done bool, err error) {
	...
	if r.geminiLogger != nil {
		r.geminiLogger.StartEntry(prompt)
	}

	geminiFlags := r.task.GeminiFlags

	// Determine gemini command: CLI override > task-level > global
	geminiCmd := r.opts.GeminiCommand
	if geminiCmd != "" {
		if r.opts.Verbose {
			fmt.Printf(ColorInfo("Using CLI override gemini_command: %s\n"), geminiCmd)
		}
	} else {
		geminiCmd = r.task.GeminiCommand
		if geminiCmd != "" && r.opts.Verbose {
			fmt.Printf(ColorInfo("Using task-level gemini_command: %s\n"), geminiCmd)
		}
	}
	if geminiCmd == "" {
		geminiCmd = r.env.Config.GeminiCommand
	...
	inactivityTimer := NewDelayedProgressTimer("Waiting for Gemini...", 30*time.Second)

	fmt.Println(ColorInfo("Running Gemini..."))
	...
	geminiOutput, err := RunGeminiCommand(geminiCmd, geminiFlags, prompt, r.env.ProjectDir, r.geminiLogger, timeout, streamCb)
	...
	if r.geminiLogger != nil {
		r.geminiLogger.EndEntry()
	}

	// Check for rate limit in output
	if strings.Contains(geminiOutput, rateLimitPhrase) {
		return false, &rateLimitError{msg: "gemini rate limit hit"}
	}
	...
	if err != nil {
		...
		return false, fmt.Errorf("gemini failed: %w", err)
	}

	// Verify build FIRST before checking candidate presence
	if !r.runVerify() {
		fmt.Println(ColorWarning("Build failed after Gemini changes"))
...
func (r *Runner) logOutcome(outcome Outcome, details string) {
	if r.geminiLogger != nil {
		r.geminiLogger.LogOutcome(outcome, details)
	}
}
