...
// GeminiLogger handles logging of Gemini interactions.
type GeminiLogger struct {
	file      *os.File
	startTime time.Time
}

// NewGeminiLogger creates a new logger for Gemini interactions.
func NewGeminiLogger(taskDir string) (*GeminiLogger, error) {
	path := filepath.Join(taskDir, "gemini.log")
	file, err := os.OpenFile(path, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		return nil, fmt.Errorf("failed to open gemini log: %w", err)
	}

	return &GeminiLogger{file: file}, nil
}

// StartEntry begins a new log entry with timestamp and prompt.
func (l *GeminiLogger) StartEntry(prompt string) error {
...
func (l *GeminiLogger) LogOutcome(outcome Outcome, details string) error {
...
func (l *GeminiLogger) EndEntry() error {
	_, err := fmt.Fprintf(l.file, "%s\n", separator)
	return err
}

// Write implements io.Writer for streaming Gemini output to the log.
func (l *GeminiLogger) Write(p []byte) (n int, err error) {
	return l.file.Write(p)
}

// Close closes the log file.
func (l *GeminiLogger) Close() error {
	if l.file != nil {
		return l.file.Close()
	}
	return nil
...
func (l *GeminiLogger) Path() string {
	return l.file.Name()
}
