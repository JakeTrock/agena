...
func TestStartupBanner(t *testing.T) {
	...
	// Should contain bold "Agena"
	if !strings.Contains(result, colorBold+"Agena") {
		t.Error("Startup banner should contain bold 'Agena'")
	}

	// Should contain star ASCII art elements
	if !strings.Contains(result, "/") || !strings.Contains(result, "\\") {
		t.Error("Startup banner should contain star ASCII art")
	}

	// Should contain cyan color
	if !strings.Contains(result, colorCyan) {
		t.Error("Startup banner should use cyan color for star")
	}
}

func TestStartupBannerDryRun(t *testing.T) {
	result := StartupBanner("my-task", "/path/to/gemini.log", "dry-run")
	...
	if !strings.Contains(result, "Logs:") || !strings.Contains(result, "gemini.log") {
